...
import type { _SERVICE as EventsService } from "../declarations/events/events.did.d.ts"
import type { _SERVICE as TicketService } from "../declarations/ticket/ticket.did.d.ts"
import type { _SERVICE as ProfileService } from "../declarations/profile/profile.did.d.ts"
...
import type { _SERVICE as TicketServiceType } from "../declarations/ticket/ticket.did.d.ts"
...
export class IcApi {
  ...
  async updateIdentity(identity: any): Promise<void> {
    ...
    try {
      ...
      // For local development, we need to fetch the root key only in development mode
      if (
        (this.host.includes("localhost") || this.host.includes("127.0.0.1")) &&
        process.env.NODE_ENV === "development"
      ) {
        await this.agent.fetchRootKey().catch((err) => {
          console.warn("Unable to fetch root key (likely okay in dev):", err)
        });
      }
  ...
  async getAllEvents(): Promise<Event[]> {
    ...
    try {
      ...
      if (typeof response === "object" && response !== null && "ok" in response && response.ok !== undefined) {
  ...
  async createEvent(request: CreateEventRequest): Promise<Result<bigint, Error>> {
    ...
    try {
      ...
      if (typeof response === "object" && response !== null && "ok" in response && response.ok !== undefined) {
        ...
        const err = (response as any).err as any
  ...
  async verifyTicket(ticketId: bigint): Promise<Result<boolean, Error>> {
    ...
    try {
      const response = await this.ticketActor.verifyTicket({ ticketId: ticketId.toString() })

      if (typeof response === "object" && response !== null) {
        if ("ok" in response && typeof response.ok === "boolean") {
          return { ok: response.ok }
        } else if ("err" in response && typeof response.err === "object") {
